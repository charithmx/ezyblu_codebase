...
$(document).ready(function () {
    ...
    $('#detailsTableBody').on('click', 'a.detailsButton', function () {
        var id = $(this).data('id');
        $.ajax({
            type: 'GET',
            url: 'common/get_details.php',
            data: { id: id },
            success: function (response) {
                $('.modal-body').html(response);
                $('#modal_above_type_1').modal('show');
            },
            error: function () {
    ...
    $('#detailsTableBody').on('click', 'a.detailsButtonEdit', function () {
	 ...
	 $('#detailsTableBody').on('click', 'a.detailsButtonDelete', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                $('.modal-body').html(response);
                $('#modal_delete_type_1').modal('show');
            },
            error: function () {
                alert('Error fetching details.');
            }
