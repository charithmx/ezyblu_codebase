...
<div class="wrapper">
                ...
                <table class="table table-hover text-wrap">
				  ...
				  if(isset($_POST['add_activity'])){
					  ...
					  //--------- Setting Check start
						...
						if($set == "Sequential"){ //--Sequential Start
							...
							$query_activity = "INSERT INTO individual_m1_core_activity(core_process_id, sub_process_id, core_process_ref, system_activity_id, activity_ref, activity_ref_next, activity, role, project_id, owner, settings, date) VALUES ('$core_process_id', '', '$core_process_ref', '$system_activity_id', '$this_activity_ref', '$next_activity_ref', '$this_activity', '$e_role', '$project_id', '$username', '1', '$today')";
						...
						elseif($set == "Usecase"){  //--Usecase Start
							...
							//---- Activity Entry Query Start
							$query_activity = "INSERT INTO individual_m1_core_activity(core_process_id, sub_process_id, core_process_ref, system_activity_id, activity_ref, activity_ref_next, activity, role, project_id, owner, settings, date) VALUES ('$core_process_id', '', '$core_process_ref', '', '$usecase_activity_ref', '', '$this_activity', '$e_role', '$project_id', '$username', '3', '$today')";
							$result_activity = mysqli_query($con_global, $query_activity);
							//---- Activity Entry Query End
				  ...
				  if(isset($_POST['edit_activity'])){					  
				   ...
				   if(isset($_POST['delete_activity'])){					  
						...
						$project_id = $_POST['project_id'];
						//---- Activity Edit Query Start
						$query_edit = "DELETE FROM individual_m1_core_activity WHERE id='$id'";
						$result_edit = mysqli_query($con_global, $query_edit);
						//---- Activity Edit Query End
						$listing = "edit";
				  }
				  if(isset($_POST['add_condition'])){ ///---- condition start
				  		$today = date('Y-m-d h:i:s');
						include('common/add_condition.php');
						$listing = "condition";	
				  ...
				  //--------From the begining - Start
						...
						if(mysqli_num_rows($result)>0){  //--- Core Process Available
							while($row = mysqli_fetch_array($result)){		
								...
								$query_act = "Select * from individual_m1_core_activity where core_process_id = '$core_process_id' order by id ASC";
								...
								if(mysqli_num_rows($result_act)>0){ 
									...
									//	$act_id = $row_act['id']; 
										...
										echo '<tr>
											  <td'.$css_settings.'>'.$core_process_ref.'.'.$activity_ref.'</td>
											  <td'.$css_settings.'>'.$activity.'</td>
											  <td'.$css_settings.'>'.$activity_ref_next.'</td>
											  <td>';
							...
							echo '<script type="text/javascript">
								//	window.locationn = "add_core.php?p='.$project_id.'";
								</script>';			
						}
						$next_activity_ref = $this_system_activity_id + 1;
				  ?>
                  <form action="build_act.php" method="post" enctype="multipart/form-data">
                  <tr id="myDiv">
                     ...
                     <input type="hidden" name="project_id" value="<?php echo $project_id; ?>" />
                      ...
                      <td><input type="text" class="form-control" name="next_activity_ref" value="<?php echo $core_process_ref.'.'.$next_activity_ref; ?>"></td>
  ...
  <!-- Bootstrap Modal -->
<?php include('common/modal_above_type_1.php'); ?> 
...
count = count + 1;
    $(document).ready(function(){
       ...
       <div class="modal fade" id="modal-default-con">
...
if(isset($_GET['above'])){
  ...
  <footer class="main-footer">
    ...
    <strong>Copyright &copy; 2024 <a href="https://ezyblu.com">ezyBLU</a>.</strong> All rights reserved.
  </footer>
